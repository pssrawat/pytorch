...
class AOTInductorTestsTemplate:
    ...
    def test_aoti_debug_printer_codegen(self):
        if not IS_FBCODE and self.device == "cpu":
            raise unittest.SkipTest("aoti debug printer is not supported on cuda")
...
if not IS_FBCODE:
    ...
    CUDA_TEST_FAILURES.update(
        {
            "test_aoti_debug_printer_codegen": fail_abi_compatible_cuda(is_skip=True),
        }
    )
